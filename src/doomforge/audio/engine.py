...
from doomforge.audio.analysis import analyze_buffer
...
class MasteringEngine:
    def analyze(self, path: str) -> AudioMetrics:
        audio, sr = read_audio(path, duration_s=300.0)
        return analyze_buffer(audio, sr, path=path)
    ...
    def render(self, source_path: str, output_path: str, settings: MasteringSettings) -> RenderReport:
        # Decode once. This avoids a second FFmpeg temp lifecycle on compressed
        # sources and removes an important Windows file-lock race.
        audio, sr = read_audio(source_path)
        source_metrics = analyze_buffer(audio, sr, path=source_path)
        processed = process_buffer(audio, sr, settings)
